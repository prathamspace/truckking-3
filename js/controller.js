		...
		NinzioSlider.prototype.init = function(){
			...
			// ====================================================
				...
				var slider           = this.element,
					...
					canvasContent    = slider.find('.canvas-content'),
			...
			// ====================================================
				...
				var optAutoplay      = slider.data('autoplay'),
					...
					optAutoHeight    = slider.attr('data-autoheight'),
					optEffect        = slider.data('effect');
			...
			// ====================================================
				...
				function OffsetValues(){
					var canvasWidth  = slide.find('.slider-canvas').first().width(),
						...
						offsetArray  = [canvasWidth, sliderWidth, Math.round((sliderWidth - canvasWidth)/2), sliderHeight];
					return offsetArray;
				}
				...
				function MouseMoveSlide(){
					if (optEffect == "mousemove") {
						slider.find('.mousemove').each(function(){
							$(this).css({
								'height':Math.round(responsiveValues[3]*1.2)+'px',
								'width':Math.round(responsiveValues[1]*1.2)+'px',
								'margin-top':-Math.round(responsiveValues[3]*0.2/2)+'px',
								'margin-left':-Math.round(responsiveValues[1]*0.2/2)+'px',
							});
						});
					}
				}
				...
				function ParallaxSlide(){
					if (optEffect == "parallax") {

						var parallaxRatio   = 1.2;
						var parallaxPercent = 0.3;
						...
						slider.find('.parallax-container').each(function(){
							var $this = $(this);
				...
				function MouseMove(element,speed,direction) {
					slider.find('ul.ninzio-slides').mousemove(function(e){
				...
				function ResetMouseMove(){
					slide.eq(active).find('.mousemove').css({'top':'0px','left':'0px'});
					slide.eq(active).find('.layer-mousemove').each(function(){
				...
				function Navigate(direction){
					...
					$('.slide-count').each(function(){
						$(this).find('.current').html(active+1);
					});
				...
				function Autoheight(){
					if (optAutoHeight == "true") {
						if (Modernizr.mq("only screen and (min-width: 1025px)")) {
							var winHeight = window.innerHeight;
							canvasContent.css({
								'-webkit-transform':'translateY('+Math.round((winHeight-optHeight)/2)+'px)',
								'-ms-transform':'translateY('+Math.round((winHeight-optHeight)/2)+'px)',
								'transform':'translateY('+Math.round((winHeight-optHeight)/2)+'px)',
							});
						}
					};
				}
			...
			imagesLoaded( slider, function() {
				...
				$.when(LayerCoords(),MouseMoveSlide(),ParallaxSlide()).done(
					function(){
						setTimeout(function(){

								if (optEffect == "mousemove") {
									MouseMove(slider.find('.mousemove'), 10, -1);
								};
							...
							if(totalSlides > 1 && optArrows == true){
								$('<span data-direction="prev" class="controls prev slider-nav icon-arrow-left2"><span class="slide-count left"><span class="current">'+( active +1 )+'</span><span class="general">'+totalSlides+'</span></span></span><span data-direction="next" class="controls next slider-nav icon-arrow-right2"><span class="slide-count right"><span class="current">'+( active +1 )+'</span><span class="general">'+totalSlides+'</span></span></span>').appendTo(slider);
							}
							...
							slider.find('.sk-cube-grid').fadeOut(400, function(){
			...
			$(window).resize(function(){
				...
				delay_exec('resizeCorrections', 500, function(){
			        responsiveValues = OffsetValues();
					LayerCoords();
					MouseMoveSlide();
					ParallaxSlide();
...
/*======================*/
	...
	(function($){
		...
		$('.mob-header .ls > div > ul > li > a').off('click').on('click',function(event){
			event.preventDefault();
			$('.mob-header .ls').toggleClass('animated');
		...
		$('.mob-menu ul li a > .di').on("click", function(event){
			$(this).toggleClass('animate');
		...
		$('.mob-menu-toggle').on("click",function(event){
			...
			$('.mob-header-content').toggleClass('animate').stop().slideToggle(300, "easeOutQuart");
		});
		...
		// Header effects
			...
			$('.desk-menu > ul > li').hoverIntent(
				function(){
					if ($(this).attr('data-mm') == "true") {
						$(this).children('ul').stop(true, true).animate({'opacity':'1'}, 400, "easeOutCirc").css('display','table');
					} else {
						$(this).children('ul').stop(true, true).animate({'opacity':'1'}, 400, "easeOutCirc").css('display','block');
					}
				},
				function(){
					$(this).children('ul').stop(true, true).animate({'opacity':'0'}, 400, "easeOutCirc", function(){
						$(this).css('display','none');
					});
				}
			);

			$('.desk-menu > ul > li > ul > li').hoverIntent(
				function(){
					$(this).children('ul').stop(true, true).animate({'opacity':'1'}, 400, "easeOutCirc").css('display','block');
				},
				function(){
...
/*======================*/
	...
	(function($){
		...
		$('.woocommerce .single-product-image .images').each(function(){

			var $this = $(this),
				thumbs = $this.find('.thumbnails > a'),
				thumbsQ = thumbs.length,
				galleryImage = $this.find('.woocommerce-main-image');
		...
		$('.loop .product, .nz-products .product').each(function(){
			...
			if (!addToCard.hasClass('added')) {
				...
				addToCard.on('click',function(){
					...
					productProgress.fadeIn(400,function(){
						...
						setTimeout(function(){
							$thisProgress.fadeOut(400,function(){
								$this.addClass('added');
							});
						}, 500);
...
/*======================*/
	...
	(function($){
        ...
        animateInView($('.nz-recent-portfolio[data-animate="true"] .nz-portfolio-posts'),250);
        ...
        animateInView($('.loop .animation-true .port-post'),250);
...
/*======================*/
	...
	(function($){
		...
		/*-----------------------*/
			...
			$('.loop .blog-post').each(function(){
				var $this = $(this);
				var sizer = $this.find('.post');
			...
			$('ul.products').each(function(){
				...
				$container.parent().find('.nz-product-filter .filter').on('click', function() {
					var $this    = $(this),
						...
						if ( !isActive ) {$('.nz-product-filter .active').removeClass('active');}
